...
GcmProvider.prototype.pushNotification = function(notification, deviceToken) {
  ...
  var registrationTokens = (typeof deviceToken == 'string') ?
    [deviceToken] : deviceToken;
  var message = this._createMessage(notification);

  debug('Sending message to %j: %j', registrationTokens, message);
  this._connection.send(message,  { registrationTokens: registrationTokens }, 3, function(err, result) {
    if (!err && result && result.failure) {
      ...
      result.results.forEach(function(value, index) {
        ...
        if  (code === 'NotRegistered' || code === 'InvalidRegistration') {
          debug('Device %j is no longer registered.', registrationTokens[index]);
          devicesGoneRegistrationIds.push(registrationTokens[index]);
        } else if (code) {
          errors.push(g.f('{{GCM}} error code: %s, deviceToken: %s',
            (code || 'Unknown'), registrationTokens[index]));
        }
      });
